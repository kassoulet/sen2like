...
class Sentinel2Product(S2L_Product):
    ...
    def band_files(self, band):
        ...
        if self.mtl.data_type == "Level-2A":
            files = glob.glob(
                os.path.join(
                    band_path, _L2A_BAND_FOLDER[band], f"*_{band}_{_L2A_BAND_FOLDER[band][1:]}{self.mtl.file_extension}"
                )
            )
            return files
    ...
    def _set_psd(self) -> str:
        ...
        if self.mtl.processing_sw < min_supported:
            logger.error("Unsupported Baseline: %s, minimum supported is %s", self.mtl.processing_sw, min_supported)
