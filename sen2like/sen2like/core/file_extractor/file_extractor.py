...
class InputFileExtractor(abc.ABC):
    ...
    def _apply_roi(self, image_masks: ImageMasks, roi_file_path: str) -> ImageMasks:
        ...
        for mask in [image_masks.validity_mask, image_masks.no_data_mask]:
            ...
            try:
                dataset = gdal.Warp(mask_dest_path, src_mask_dataset, options=options)
...
class S2FileExtractor(InputFileExtractor):
    ...
    def _create_masks_from_scl(self, mask_filename: str, res: int) -> ImageMasks:
        ...
        if scl.xRes != res:
            shape = (
                int(scl_array.shape[0] * -scl.yRes / res),  # pylint: disable=invalid-unary-operand-type
    ...
    def _create_valid_mask_form_l1c_gml(self, mask_filename: str, nodata_mask: MaskImage, res: int) -> MaskImage:
        ...
        if nb_cloud > 0:
            output_bounds = [
                self._input_product.ULX,
                self._input_product.LRY,
                self._input_product.LRX,
                self._input_product.ULY,
            ]
            ...
            gdal.Rasterize(
                mask_filename,
                self._input_product.cloudmask,
                outputType=gdal.GDT_Byte,
                creationOptions=["COMPRESS=LZW"],
                burnValues=0,
                initValues=1,
                outputBounds=output_bounds,
                outputSRS=self._input_product.epsg,
                xRes=res,
                yRes=res,
            )
    ...
    def _create_valid_mask_form_l1c_jp2(
        self, mask_filename: str, image: S2L_ImageFile, nodata: np.ndarray, res: int
    ) -> MaskImage:
    ...
    def _get_valid_pixel_mask(self, mask_filename: str) -> ImageMasks:
        ...
        if self._input_product.scene_classif_band:
            image_masks = self._create_masks_from_scl(mask_filename, self._input_product.mask_resolution)
        # L1C case for instance -> No SCL, but NODATA and CLD mask
        else:
            log.debug("L1C Case")
            image_masks = self._create_masks_from_l1c(mask_filename, self._input_product.mask_resolution)
...
class LandsatFileExtractor(InputFileExtractor):
    ...
    def _create_masks_from_scl(self, mask_filename: str) -> ImageMasks:
        ...
        if scl.xRes != res:
            shape = (
                int(scl_array.shape[0] * -scl.yRes / res),
                int(scl_array.shape[1] * scl.xRes / res),
    ...
    def _get_image_for_angle(self, out_file, downsample_factor) -> str:
        ...
        if self._input_product.data_type == "L2TP":
            ...
            log.error("Unable to get band image from %s to extract angles image", self._input_product.product_name)
            raise Exception("Unable to get band image to extract angles image")

        image = self._input_product.reflective_band_list[0]
        return downsample_coarse_image(image, os.path.dirname(out_file), downsample_factor)
    ...
    def get_angle_images(self, out_file: str = None) -> str:
        ...
        coarse_res_image = self._get_image_for_angle(os.path.dirname(out_file), downsample_factor)
...
class S2MajaFileExtractor(InputFileExtractor):
    ...
    def _create_nodata_mask(self, nodata_mask_file_path: str, mask_band_id: str, resolution_id: str) -> MaskImage:
        ...
        edge = S2L_ImageFile(
            os.path.join(self._input_product.product_path, self._input_product.edge_mask[resolution_id])
        )
        edge_arr = edge.array

        defective = S2L_ImageFile(
            os.path.join(self._input_product.product_path, self._input_product.nodata_mask[mask_band_id])
        )
    ...
    def _create_valid_pixel_mask(
        self, mask_filename: str, nodata: np.ndarray, mask_band_id: str, resolution_id: str
    ) -> MaskImage:
        ...
        """
        cloud = S2L_ImageFile(
            os.path.join(self._input_product.product_path, self._input_product.cloud_mask[resolution_id])
        )
        cloud_arr = cloud.array
        saturation = S2L_ImageFile(
            os.path.join(self._input_product.product_path, self._input_product.saturation_mask[mask_band_id])
        )
    ...
    def _get_valid_pixel_mask(self, mask_filename: str) -> ImageMasks:
        ...
        no_data_mask = self._create_nodata_mask(nodata_mask_filename, mask_band_id, resolution_id)
...
class LandsatMajaFileExtractor(InputFileExtractor):
    ...
    def _sunAnglesForExtent(self, img_info):
        ...
        pts = np.array([[ul_long, ul_lat], [ur_long, ur_lat], [ll_long, ll_lat], [lr_long, lr_lat]])
        long_deg = pts[:, 0]
        lat_deg = pts[:, 1]
