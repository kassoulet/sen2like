...
class S2LProductMtdWriter(XmlWriter, abc.ABC):
    ...
    def manual_replaces(self, product: S2L_Product):
        ...
        change_elm(
            self.root_out,
            rpath="./General_Info/Product_Info/PRODUCT_URI",
            new_value=metadata.mtd.get(f"product_{self.H_F}_name"),
        )
        change_elm(self.root_out, rpath="./General_Info/Product_Info/PROCESSING_LEVEL", new_value=f"Level-2{self.H_F}")
        change_elm(
            self.root_out, rpath="./General_Info/Product_Info/PROCESSING_BASELINE", new_value=version.baseline_dotted
        )

        generation_time = (
            datetime.strftime(metadata.mtd.get("product_creation_date"), "%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
        )  # -3 to keep only 3 decimals
        ...
        change_elm(
            self.root_out,
            rpath=GRANULE_PATH,
            new_value=self.IMAGE_FORMAT[config.get("output_format")],
            attr_to_change="imageFormat",
        )
        ...
        for band_id in range(0, 13):
            create_child(
                self.root_out,
                rpath=boa_offset_list_elem,
                tag="BOA_ADD_OFFSET",
                text=str(-offset),
                attribs={"band_id": str(band_id)},
            )
...
class Sentinel2ToS2LProductMtdWriter(S2LProductMtdWriter):
    ...
    def _process_product_info(self, product: S2L_Product):
        ...
        # ------------
        elements_to_copy = [
            "./General_Info/Product_Info/Datatake",
        ...
        change_elm(self.root_out, rpath="./General_Info/Product_Info/PRODUCT_TYPE", new_value=f"S2MSI2{self.H_F}")
    ...
    def _process_product_image_characteristics(self):
        ...
        copy_elements(
            ["./General_Info/Product_Image_Characteristics/Reflectance_Conversion/U"], self.root_in, self.root_out
        )
        ...
        if classif_element:
            ...
            copy_children(self.root_in, SCENE_CLASSIFICATION_LIST_PATH, self.root_out, SCENE_CLASSIFICATION_LIST_PATH)
    ...
    def _process_auxiliary_data_info(self):
        ...
        copy_children(self.root_in, GIPP_LIST_PATH, self.root_out, GIPP_LIST_PATH)
...
class LandsatToS2LProductMtdWriter(S2LProductMtdWriter):
    ...
    def _specific_replaces(self, product: S2L_Product):
        ...
        change_elm(
            self.root_out, rpath="./General_Info/Product_Info/PRODUCT_TYPE", new_value=f"{product.sensor}OLI2{self.H_F}"
        )
        change_elm(
            self.root_out, rpath="./General_Info/Product_Info/Datatake/SPACECRAFT_NAME", new_value=product.mtl.mission
        )
        change_elm(
            self.root_out, rpath="./General_Info/Product_Info/Datatake/DATATAKE_SENSING_START", new_value=acq_date
        )
        change_elm(
            self.root_out,
            rpath="./General_Info/Product_Info/Datatake/SENSING_ORBIT_NUMBER",
        ...
        change_elm(
            self.root_out,
            rpath="./General_Info/Product_Image_Characteristics/Reflectance_Conversion/U",
            new_value=str(U),
        )
        ...
        change_elm(
            self.root_out, "./Quality_Indicators_Info/Cloud_Coverage_Assessment", new_value=product.mtl.cloud_cover
        )
...
class S2LTileMtdWriter(XmlWriter, abc.ABC):
    ...
    def manual_replaces(self, product: S2L_Product):
        ...
        angles_path = os.path.join(
            "GRANULE", metadata.mtd.get(f"granule_{self.H_F}_name"), "QI_DATA", metadata.mtd.get("ang_filename")
        )
        ...
        for ql in metadata.mtd.get(f"quicklooks_{self.H_F}"):
            ...
            create_child(
                self.root_out, QUALITY_INDICATOR_PATH, tag=f"{os.path.splitext(ql_name)[0]}_FILENAME", text=ql_path
            )
...
class Sentinel2ToS2LTileMtdWriter(S2LTileMtdWriter):
    ...
    def _specific_replaces(self, product: S2L_Product):
        ...
        copy_children(self.root_in, IMAGE_CONTENT_QI_PATH, self.root_out, IMAGE_CONTENT_QI_PATH)
...
class LandsatToS2LTileMtdWriter(S2LTileMtdWriter):
    ...
    def _specific_replaces(self, product: S2L_Product):
        ...
        change_elm(
            self.root_out,
            "./General_Info/Archiving_Info/ARCHIVING_TIME",
        ...
        change_elm(
            self.root_out, "./Geometric_Info/Tile_Angles/Mean_Sun_Angle/ZENITH_ANGLE", new_value=str(zenith_angle)
        )
        change_elm(
            self.root_out, "./Geometric_Info/Tile_Angles/Mean_Sun_Angle/AZIMUTH_ANGLE", new_value=str(azimuth_angle)
        )
        change_elm(
            self.root_out,
        ...
        # -----------------------
        self.remove_children(IMAGE_CONTENT_QI_PATH)
        create_child(self.root_out, IMAGE_CONTENT_QI_PATH, tag="CLOUDY_PIXEL_PERCENTAGE", text=product.mtl.cloud_cover)
