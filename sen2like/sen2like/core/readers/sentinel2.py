    ...
    def __init__(self, product_path, mtd_file=None):
        ...
        try:
            ...
            for quantification_node_name in [
                ...
                try:
                    self.quantification_value = node_value(dom, quantification_node_name)
                    break
                except IndexError:
                    pass
            ...
            for cpt, node in enumerate(nodes):
                ...
                self.radio_coefficient_dic[str(cpt)] = {
                    "Band_id": str(band_id),
                    "Gain": 0.00001,
                    "Offset": 0.0,
        ...
        if tile_rep is not None:
            ...
            try:
                ...
                for cpt, sub_node in enumerate(sub_nodes):
                    ...
                    self.viewing_azimuth_angle[str(cpt)] = {
                        "Band_id": str(band_id),
                        "VAA": node_value(sub_node, "AZIMUTH_ANGLE"),
                    }
                    self.viewing_zenith_angle[str(cpt)] = {
                        "Band_id": str(band_id),
